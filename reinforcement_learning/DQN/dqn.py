...
class DQN(tf.keras.Model):
    def __init__(self, input_dim, num_action, name):
...
class ActiveTargetDQN(tf.keras.Model):
    ...
    def __init__(self, input_dim, num_action, memory_size, gamma, learning_rate=1e-3, device_name="cpu:0",
        ...
        super(ActiveTargetDQN, self).__init__()
        self.input_dim = input_dim
        self.num_action = num_action
        ...
        # active layer
        self.q_active = DQN(input_dim, num_action, name="q_active")
        self.q_target = DQN(input_dim, num_action, name="q_target")
    ...
    def copy_active2target(self):
        ...
        """
        for idx_weight in range(len(self.q_active.variables)):
            tf.assign(self.q_target.variables[idx_weight], self.q_active.variables[idx_weight])
    ...
    def grad(self, X, action, r, X_next, done):
        ...
        return tape.gradient(loss_val, self.q_active.variables), loss_val
    ...
    def train(self, X, action, reward, X_next, done):
        ...
        with tf.device(self.device_name):
            ...
            self.optimizer.apply_gradients(zip(grads, self.q_active.variables))
